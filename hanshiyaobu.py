...
if __name__ == '__main__':
    try:
        ...
        for weblist in web_data[1]["GuoNei"][0:1]:
            ...
            try:
                url = "https://impfood.mfds.go.kr/CFCEE01F01/getList?page=1&limit=10&cntntsSn=&srchClassCd=&searchCondition=pdNm&searchInpText=&btnSearch="
                driver.get(url)
                time.sleep(2)
                titles = driver.find_element(By.CSS_SELECTOR, "#frm > fieldset > div.gallery.type2.txt-type.long-title > ul > li:nth-child(1) > h4 > strong")

                title = titles.text
                print("newis是" +title)
                print("oldis是" + weblist["title"])
                countytitles = driver.find_element(By.CSS_SELECTOR, "#frm > fieldset > div.gallery.type2.txt-type.long-title > ul > li:nth-child(1) > dl > dt > ul > li:nth-child(3) > span")
                countytitle = countytitles.text
                print(countytitle)
                countyCHINA = "중국"
                if title != weblist["title"] and countytitle  == countyCHINA:
                    weblist["title"] = title
